            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    @if ($suppliers->isEmpty())
                        <p class="text-sm text-gray-500 dark:text-gray-400">No hay proveedores.</p>
                    @else
                            ...
                            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700 text-sm">
                                <thead class="bg-gray-50 dark:bg-gray-700">
                                    <tr>
                                        ...
                                        <th class="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-300">Contacto</th>
                                ...
                                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                                    @foreach ($suppliers as $supplier)
                                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                                            ...
                                            <td class="px-4 py-3">{{ $supplier->suppliers_rfc }}</td>
                                            <td class="px-4 py-3">
                                                <div class="flex items-center gap-3">
                                                    <a href="{{ route('suppliers.show', $supplier) }}" class="text-indigo-600 dark:text-indigo-400 hover:underline">Ver</a>
                                                    <a href="{{ route('suppliers.edit', $supplier) }}" class="text-blue-600 dark:text-blue-400 hover:underline">Editar</a>
                                                    <form method="POST" action="{{ route('suppliers.destroy', $supplier) }}" onsubmit="return confirm('¿Eliminar el proveedor "{{ $supplier->name }}"?')">
                                                        @csrf
                                                        @method('DELETE')
